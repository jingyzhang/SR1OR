...
class BaseLearner(object):
    ...
    def eval_task(self):
        y_pred, y_true = self._eval_cnn(self.test_loader)   # 具体验证的代码用来获得预测结果和标签
        ## 这部分其实是模型直接预测包含了新类和先前所有类的 topk=1 预测 ，比如现在是任务1(类别2和类别3)，那这里的标签最大其实是3，
        # 也就是说我这里其实不是根据任务id去计算的，直接遍历先前以及当前任务的所有数据，此时分类头的channel是4，batch为128的话，那这里预测头的结果就是128,4，直接选择最大的概率去预测类别
        # print('y_pred', y_pred)
        # print('y_true', y_true)
    ...
    def _eval_cnn(self, loader):
        ...
        for _, (_, inputs, targets) in enumerate(loader):
            ...
            # print(self.topk)
            y_pred1 = torch.tensor(predicts)
            y_true1 = torch.tensor(targets)
            # print("预测预测预测预测预测预测预测", y_pred1)
            # print("标签标签标签标签标签标签标签", y_true1)
