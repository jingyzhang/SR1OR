...
def get_backbone(args, pretrained=False):
    ...
    if name == "pretrained_vit_b16_224" or name == "vit_base_patch16_224":
        ...
        logging.info('Load pretrain model {} from timm'.format('vit_base_patch16_224'))
...
class IncrementalNet(BaseNet):
    ...
    def save_fc(self, filename, task_id):
        # pass
        torch.save(self.fc.weight.detach(), os.path.join(filename, 'CLs_weight'+str(task_id)+'.pt'))
        torch.save(self.fc.bias.detach(), os.path.join(filename, 'CLs_bias'+str(task_id)+'.pt'))
        logging.info('*** Save fc parameter to ' + os.path.join(filename, 'CLs_weight'+str(task_id)+'.pt'))
        logging.info('*** Save fc parameter to ' + os.path.join(filename, 'CLs_bias'+str(task_id)+'.pt'))


    def load_fc(self, filename, task_id):
        print('task_id', task_id)
        self.fc = self.generate_fc(self.feature_dim, (task_id+1)*10).cuda()
        for i in range(task_id+1):
            for j in range(i,task_id+1):
                temp_weights = torch.load(os.path.join(filename, 'CLs_weight'+str(j)+'.pt'))
                temp_bias = torch.load(os.path.join(filename, 'CLs_bias'+str(j)+'.pt'))
    ...
    def update_fc(self, nb_classes):
        ...
        # print("分类器信息：", self.feature_dim, nb_classes)
        if self.fc is not None:
            ...
            logging.info('Update FC with existing layers')
        else:
            logging.info('Initialize FC')
        del self.fc
        self.fc = fc
    ...
    def forward(self, x, ortho_loss=False, eval=False):
        if eval:
            ...
            if self.model_type == 'cnn':
                ...
                x, ortho_loss, l2loss = self.backbone(x, loss=True)
                # print("正交损失",ortho_loss)
                out = self.fc(x)
                out.update({"features": x})
                return out, ortho_loss, l2loss
            else:
